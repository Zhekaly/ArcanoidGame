...
uniform vec4 color; // multiply color (r,g,b,a)

void main(){
    vec4 t = texture(tex, uv);
    // standard alpha multiply
    FragColor = t * color;
    // If you use premultiplied alpha, adjust blending function accordingly
}
)";
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        double mouseY = WINDOW_H - ypos;
        float speed = 500.0f;
        if (keys[GLFW_KEY_LEFT]) paddle.pos.x -= speed * dt;
        if (keys[GLFW_KEY_RIGHT]) paddle.pos.x += speed * dt;
        ...
        Sprite spP;
        spP.pos = paddle.pos; spP.size = paddle.size; spP.tex = paddle.tex; spP.color = glm::vec4(1.0f); spP.transparent = false; spP.depth = 0.0f;
